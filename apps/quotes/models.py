...
class UserManager(models.Manager):
    ...
    def validate_registration(self, post_data):
        ...
        if len(post_data['first_name']) < 2 or len(post_data['last_name']) < 2 or len(post_data['quoted_by']) < 2:
        ...
        # check date field
        # if not re.match(DOB_REGEX, post_data['dob']):
...
class Quote(models.Model):
    quote_by = models.TextField(max_length=255)
    message = models.TextField(max_length=1000)
    quote = models.ForeignKey(User, related_name="quotes")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
